...
export default function HandleViewers(){
    ...
    const getEditors = async (token, currentTry) => {
        await axios.get(import.meta.env.VITE_API_ADDRESS+'/viewpermission/inv/'+organizationId+'/'+inventoryId, {
            headers: {
                Authorization: `Bearer ${token}`,
                Identity: authId,
            },
    ...
    const removePermission = async (token, receptorId, currentTry) => {
        await axios.put(import.meta.env.VITE_API_ADDRESS+'/viewpermission/inv/'+organizationId+'/'+inventoryId+'/'+receptorId, {
            status: 'inactive',
        }, {
            headers: {
                Authorization: `Bearer ${token}`,
            ...
            { ((userId !== 0 || isAuthenticated) && (accessLevel === 'admin' || accessLevel === 'owner') ) ?
                ...
                <div className="new-org-content">
                    ...
                    <div className="new-org-form">
                        ...
                        <button type="submit" className='submit-button red' onClick={() => {navigate('/organization/'+organizationId+'/')}}>Cancelar</button>
                    </div>
                </div>
                <Footer />
            </div>:
