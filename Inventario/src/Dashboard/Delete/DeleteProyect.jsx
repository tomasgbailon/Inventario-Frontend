...
export default function DeleteProy(){
    ...
    const handleSubmit = async () => {
        ...
        await axios.delete(import.meta.env.VITE_API_ADDRESS+'/proyects/'+organizationId+'/'+proyectId, {
            headers: {
    ...
    return (
        <DashboardContext.Provider value={{buttonUnlock, setButtonUnlock}}>
            {(userId !== 0 && (accessLevel === 'admin' || accessLevel === 'owner') ) ? <div className="new-org">
                <NavBar selection={1} />
                    <div className='new-org-content'>
                        <h1>¿Eliminar {orgName}?</h1>
                        <div id='small-font'>IMPORTANTE: Esta acción eliminará todos los subproyectos y productos contenidos en {orgName}</div>
                        <div className='new-org-form'>
                            <label className='orgName' >Escribe el nombre del proyecto para confirmar</label>
